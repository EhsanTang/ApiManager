...
import cn.crap.mapper.custom.CustomInterfaceDao;
import cn.crap.mapper.mybatis.InterfaceDao;
...
public class InterfaceService extends BaseService<InterfaceWithBLOBs, InterfaceDao> implements ILuceneService, IConst {
    ...
    public InterfacePDFDto getInterPDFDto(InterfaceWithBLOBs interFace, Module module, boolean escape, boolean isPdf) {
        ...
        if (interFace.getParam() != null && interFace.getParam().startsWith("form=")) {
            List<ParamDto> paramList = JSONArray.parseArray(interFace.getParam() == null ? "[]" : interFace.getParam().substring(5), ParamDto.class);
